...
Arsenal rail показывает 33 cards и фильтруется по blast, payload, terrain
destruction/creation и energy; status объясняет delivery до выстрела. Магазин
использует те же cards и отдельно показывает buy/sell quotes.
