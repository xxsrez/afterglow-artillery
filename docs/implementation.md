...
- seeded PRNG воспроизводимо сочетает macro-профиль поверхности, detail-шум,
  связные сети пещер, камеры, ответвления, входы с поверхности и bedrock;
- генератор battlefield готовит дальние безопасные spawn shelves в
  противоположных частях карты, а поиск опоры под танком учитывает полости:
  провалившийся танк оседает на ближайшем материале ниже, а не переносится на
  верхний контур;
