...
До расширения каталога нужен frame-time trace на среднем Android-устройстве.
Если измерения покажут системное ограничение fill rate или числа эффектов,
кандидатами остаются PixiJS/Phaser с WebGL. Миграция не должна менять
`lib/game/`, seed, баллистику, damage или каталог оружия.
...
До начала массового производства контента vertical slice проверяет:
...
3. шесть эффектов используют общий каталог и не меняют outcome из VFX;
...
6. performance trace на reference device остаётся до расширения каталога.
