...
- До rasterization ruleset выбирает один из четырёх тактических профилей:
  открытые холмы, центральный хребет, центральная впадина или cavern.
  `ridge` и `valley` обязаны образовывать feature высотой/глубиной не меньше
  `10%` высоты мира и шириной не меньше `160` world units.
- Танки размещаются согласованной парой с минимальным безопасным расстоянием:
  solver оценивает relief и barrier/basin между позициями, а не только
  плоскость каждой площадки отдельно. В cavern один или оба танка могут
  начинать на внутреннем полу под крышей. Такая позиция допустима только при
  свободном габарите, сплошной опоре, связном входе к поверхности и
  траектории выхода для базового Baby Missile.
- Равновзвешенное default-расписание не повторяет один профиль во всех трёх
  раундах матча. Профиль, spawn kinds, topology metrics, число попыток и
