...
- До rasterization ruleset выбирает один из четырёх тактических семейств и
  один из трёх motif внутри семейства. Всего Quick Demo использует 12
  композиций: цепь островов, ломаные плато, асимметричный склон; шпиль,
  двойные пики, mesa; глубокий бассейн, разлом, террасированный canyon;
  cliff-cave, подземная дуэль и underworld.
- Motif задаёт не только верхнюю линию, но и упорядоченные операции material
  grid: отдельные массы, открытые пустоты, арки, мосты и внутренние полки.
  Этот набор является обязательным spine, а не одной готовой картой:
  same-motif seeded instantiation существенно варьирует surface warp,
  положение/масштаб required features, bounded optional feature pack и spawn
  slots. Поэтому новый seed меняет композицию и прострелы, а не только
  шероховатость линии. Seeded noise добавляет локальную неровность после
  композиции и не выбирает её макроструктуру. Независимая legacy-генерация
  случайных caves/tunnels в Quick Demo отключена: все объёмные пустоты
  принадлежат выбранному motif.
- `ridge` и `valley` сохраняют feature высотой/глубиной не меньше `10%`
  высоты мира и шириной не меньше `160` world units. Motif со сквозной
  пропастью проверяется по фактической пустоте под мостом, а не по heightmap.
- Танки размещаются согласованной парой в motif-specific search regions:
  solver оценивает relief, barrier/basin, вертикальную асимметрию и тип опоры,
  а не только плоскость каждой площадки отдельно. Опора может быть верхней
  поверхностью основного массива, отдельным устойчивым островом или внутренним
  полом под крышей. Cave-позиция допустима только при свободном габарите,
  сплошной опоре, связном входе к поверхности и траектории выхода для
  базового Baby Missile. Для cavern seed также выбирает bounded route class:
  меняются глубина камеры, угол mouth и изгиб corridor, поэтому cave spawn не
  закреплён в одной точке на все карты motif.
- Равновзвешенное default-расписание не повторяет одно семейство во всех трёх
  раундах матча; второй seeded choice выбирает один из трёх motif семейства.
  Семейство, motif, variation candidate/signature, cavern route class, spawn
  kinds, topology/structure metrics, число попыток и fallback входят в
  детерминированные metadata. Структура измеряется и по surface silhouette, и
  по coarse 2D occupancy, чтобы caves и overhangs не исчезали из проверки.
  Решение и границы каноничности зафиксированы в
...
- короткое нажатие `←/→` поворачивает ствол на `1°` в выбранную сторону
  экрана независимо от направления танка, а `−/+` меняет силу на `10`;
  удержание любого stepper после короткой паузы повторяет действие плавно и
  ускоряется до ограниченной скорости; нажатие центрального значения открывает
  только один precision tray, а Fire до его закрытия недоступен;
